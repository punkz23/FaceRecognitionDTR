...
- [~] Task: Refactor AuthRepository for Dynamic URL
    - [ ] Write Tests: Create unit tests for `AuthRepository.register` verifying it uses the URL from `ConfigService`.
    - [ ] Implement Feature: Update `AuthRepository` to fetch the base URL from `ConfigService` before making the registration request.
